			...
			try
			{
				...
				foreach (var i in _dsOrders)
				{
					...
					bll.Add(new OrdersBLL
					{
						...
						OrderCheckouts = oc,
						DeliveryStatus =
						OStatus == 101 ? "Accepted"
						: OStatus == 102 ? "On shipping"
						: OStatus == 107 ? "Delivered"
						: OStatus == 104 ? "Cancelled"
						: OStatus == 105 ? "Refunded"
						: OStatus == 106 ? "Hold"
						: OStatus == 103 ? "Done"
						: "Other",
						UserID = i.UserID,
		...
		public RspOrdersCustomer GetOrdersAdminV2(int LocationID, string StartDate, string EndDate, string Search)
		{
			...
			try
			{
				...
				foreach (var i in _dsOrders.OrderByDescending(x => x.ID))
				{
					...
					{
						lstODM = new List<OrderModifierDetailBLL>();
						foreach (var k in _dsorderdetailmodifier.Where(x => x.StatusID == 2 && x.OrderDetailID == j.OrderDetailID))
						{
							lstODM.Add(new OrderModifierDetailBLL
							{
								StatusID = k.StatusID,
					...
					bll.Add(new OrdersBLL
					{
						...
						DeliverUserID = i.DeliverUserID,
						LocationID = i.LocationID,
						//OrderDate = i.OrderCreatedDT == null ? "" : DateTime.ParseExact(_OC.CheckoutDate, "MM/dd/yyyy hh:mm:ss", null).ToString("yyyy-MM-dd HH:mm:ss"),
						OrderDate = Convert.ToDateTime(_OC.CheckoutDate).ToString("yyyy-MM-dd HH:mm:ss"),
						OrderNo = i.OrderNo,
						OrderTakerID = i.OrderTakerID,
						OrderType = i.OrderType,
						TransactionNo = i.TransactionNo,
						...
						if (order.StatusID == 104)
						{
							try
							{
								...
								foreach (var item in getTokens)
								{
									...
									token.Title = "Vitamito | Order Status";
